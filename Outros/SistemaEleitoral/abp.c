...
int insere_abp(Abp *raiz, TElemento e) {
    ...
    if (tchcmp(e.id, (*raiz)->info.id) > 0) return insere_abp(&(*raiz)->dir, e);

    /// Se é menor que a raiz vai para esquerda
    if (tchcmp(e.id, (*raiz)->info.id) < 0) return insere_abp(&(*raiz)->esq, e);
...
/// TElemento*
int pesquisa_abp(Abp raiz, TChave id, TElemento *retorno) {
...
int altera_abp(Abp raiz, TElemento e) {
    ...
    if (tchcmp(e.id, raiz->info.id) > 0) return altera_abp(raiz->dir, e);
