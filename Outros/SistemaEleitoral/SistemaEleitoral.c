...
// ! Deve ser chamado quando o arquivo estiver apontando para a terceira linha
// do arquivo em diante
...
// ! Deve ser chamado quando arquivo estiver apontando para a primeira linha do
// arquivo; Lê o numero da estrutura. Em caso de erro, retorna 0.
...
// ! Deve ser chamado quando o arquivo estiver apontando para a segunda linha do
// arquivo.
...
int entrada_de_dados(int argc, char *argv[]) {
    ...
    Estruturas *est =
        cria_estruturas(tipo_est, vereadores, prefeitos, eleitores);
    ...
    // Printa resumo
    print_exec(obter_colisoes(est), est->hashVereadores->numColisoes,
               est->hashPrefeitos->numColisoes, tempo);
    fprint_exec(obter_colisoes(est), est->hashVereadores->numColisoes,
                est->hashPrefeitos->numColisoes, tempo);
