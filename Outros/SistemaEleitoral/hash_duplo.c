...
int hash1(int key, int tamanho_tabela) { return abs(key % tamanho_tabela); }

int hash2(int key, int np) { return (PRIME - (key % PRIME)); }
...
void print_hash_duplo(HashDuplo t) {
    for (int i = 0; i < t->tamanho; i++) {
        printf("\n%d: ", i);
        if (t->elementos[i].condicao == CHEIO) print_tch(t->elementos[i].id);
    }
}
